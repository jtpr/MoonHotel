...
void readKeywordWeights(istream &input, vector<string> &keywords, vector<double> &weights) {
    ...
    while (input >> keyString >> weightDouble){   // While a word and number are read in from fileIn
        keywords.push_back(keyString);          // add the word to vector of keywords
        weights.push_back(weightDouble);        // add num to vector of weights
    }
}
...
void readReview(istream &input, vector<string> &reviewWords) {
    ...
    while (input >> copyWords){

        reviewWords.push_back(copyWords);

    }

    preprocessReview(reviewWords);
...
double reviewScore(const vector<string> &reviewWords, const vector<string> &keywords, const vector<double> &weights) {
    ...
    vector<string> copyReview = reviewWords;

    preprocessReview(copyReview);

    for (int i = 0; i < copyReview.size(); ++i){

        string word = copyReview[i];
